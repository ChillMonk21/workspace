...
int GetLD_PRIVILIGE(int *x)  { return (x[LD_PRIVILIGE]); }
...
void eval_micro_sequencer() {
   ...
   */
    ...
    int intEn			= CURRENT_LATCHES.INT;
    int privMode		= CURRENT_LATCHES.PRIVILIGE;
...
int marMuxOutput, pcOutput, shfOutput, aluOutput, mdrOutput, spMuxOutput, psrOutput;
...
void eval_bus_drivers() {
   ...
   */
	...
	psrOutput		= Low16bits((CURRENT_LATCHES.PRIVILIGE << 15) +
					  (CURRENT_LATCHES.PRIORITY << 8) +
					  (CURRENT_LATCHES.N << 2) +
					  (CURRENT_LATCHES.Z << 1) +
					  (CURRENT_LATCHES.P));
}
...
void latch_datapath_values() {
   ...
   */
    ...
    /* Save User Stack Pointer Register*/
    if(ldUSP){
